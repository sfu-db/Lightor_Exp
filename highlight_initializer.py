...
class DataLoader:
    ...
    """
    def __init__(self, dataset_name, dataset_type, subfiles=None, **para):
        ...
        for f in self.file_list:
            try:
                ...
                true_labels[f] = list(csv.reader(open(file_path), delimiter=' '))
                self.true_labels[f] = []
                self.baseline_gt[f] = []
                self.gt[f] = []
                start_gt = []
                end_gt = []
                for j in true_labels[f]:
                    if j[0] == '000-00':
                        # no sliding window, but gt found by crowrsourcing
                        start_gt += list(range(sec(j[1]) - 10, sec(j[2]) + 1))
                        end_gt += list(range(sec(j[1]), sec(j[2]) + 11))
                        self.gt[f].append([sec(j[1]), sec(j[2])])
                    elif j[1] == '000-00' and j[2] == '000-00':
                        # no sliding window, gt generated by DL method
                        self.baseline_gt[f].append(j[0])
                    else:
                        # sliding window
                        self.baseline_gt[f].append(j[0])
                        self.true_labels[f].append(j)
                        start_gt += list(range(sec(j[1]) - 10, sec(j[2]) + 1))
                        end_gt += list(range(sec(j[1]), sec(j[2]) + 11))
                        self.gt[f].append([sec(j[1]), sec(j[2])])
                self.start_gt[f] = start_gt
                self.end_gt[f] = end_gt
        ...
        if dataset_type == 'train':
            ...
            self.true_examples = sorted(self.true_examples)
            self.false_examples = sorted(self.false_examples)
            random.seed(1)
    ...
    def _normalization(self, windows, vec):
        for j in vec:
            ...
            max_j = max(j_list)
            for k in windows:
                k[j] = float(format(float(k[j] - min_j) / float(max_j - min_j), '.10f'))
...
class Adjustment:
    ...
    def aug_max(self, t):
        ...
        re = [i[0] for i in scores if i[1] == max_score[1]]
    ...
    def train(self):
        ...
        self.const = [expand_const, avg_length]
